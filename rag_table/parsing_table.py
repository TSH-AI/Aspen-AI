"""
parsing.py
Handles PDF/Markdown parsing related functionalities.
"""
...
async def parse_single_pdf(pdf_path: Path, md_dir: Path, llama_parse_api_key: str) -> Path:
    """
    Parses a single PDF file and converts it to Markdown format.

    Args:
        pdf_path (Path): Path to the PDF file.
        md_dir (Path): Output directory for Markdown files.
    ...
    # If it already exists and is not empty, return directly.
    ...
    while retry_count < max_retries:
        try:
            # Call LlamaParse to parse the PDF (wrap sync method in a thread pool)
            ...
            if not llama_parse_documents or all(not doc.text.strip() for doc in llama_parse_documents):
                raise ValueError(f"LlamaParse returned empty content for: {pdf_path}")
    ...
    # Write failure report on failure
    async with aiofiles.open(document_path, "w", encoding='utf-8') as f:
        await f.write(f"# {pdf_filename} - Parsing Failed\n\n")
        await f.write(f"An error occurred while parsing this PDF. Last error message:\n{last_exception}\n\n")
...
async def batch_parse_pdfs(pdf_dir: Path, md_dir: Path, llama_parse_api_key: str) -> List[Path]:
    """
    Parses all PDF files in a directory in batch.

    Args:
        pdf_dir (Path): Path to the PDF directory.
        md_dir (Path): Output directory for Markdown files.
        llama_parse_api_key (str): LlamaParse API key.
    Returns:
        List[Path]: A list of paths to all generated Markdown files.
    """
